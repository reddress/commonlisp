...
(defun char-to-index (c)
  (- (char-code (char-upcase c)) 64))

(defun index-to-char (i)
  (if (= i 0)
      (code-char (+ 64 26))
      (code-char (+ 64 i))))
...
(defun repeat-key (key length)
  (let* ((key-length (length key))
         (repetitions (ceiling length key-length))
         (repeated-key (repeat-str key repetitions)))
    (subseq repeated-key 0 length)))

(defun normalize-string-iter (s out)
  (if (= (length s) 0)
      (coerce (reverse out) 'string)
      (normalize-string-iter (subseq s 1)
                             (let* ((head (char-upcase (char s 0)))
                                    (head-index (char-to-index head)))
                               (if (and (>= head-index 1) (<= head-index 26))
                                   (cons head out)
                                   out)))))

(defun normalize-string (s)
  (normalize-string-iter s '()))

(defun vigenere-encode (s k)
